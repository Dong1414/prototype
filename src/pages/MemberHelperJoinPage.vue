  ...
  setup(props) {
    ...
    function checkAndJoin() {     
      ...
      const startJoin = (genFileIdsStr:string) => {
        helperjoin(loginIdEl.value, loginPwEl.value, nameEl.value, cellphoneNoEl.value, emailEl.value, genFileIdsStr, sidoEl.value, careerEl);
      };
    ...
    function helperjoin(loginId:string, loginPw:string, name:string, cellphoneNo:string, email:string, genFileIdsStr:string, sidoEl:string, careerEl:string) {
      mainApi.member_doHelperJoin(loginId, loginPw, name, cellphoneNo, email, genFileIdsStr, sidoEl, careerEl)
        .then(axiosResponse => {
          alert(axiosResponse.data.msg);
          if ( axiosResponse.data.fail ) {
            return;
